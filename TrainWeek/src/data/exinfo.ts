...
export interface ExInfo { m: string[]; alt: string[] }

export const EXINFO: Record<string, ExInfo> = {
  /* ---------------- shared prep / warm-up ---------------- */
  ...
  'Hip thrust, dumbbell or barbell': { m: ['glutes', 'hamstrings'], alt: ['Glute drive machine', 'Single-leg hip thrust', 'Barbell glute bridge'] },
  ...
  'Cable kickback': { m: ['glutes'], alt: ['Banded kickback', 'Step-up, glute focus', 'Single-leg hip thrust'] },
  ...
  'Rope pushdown': { m: ['triceps'], alt: ['Straight-bar pushdown', 'Bench dips', 'Dumbbell kickback'] },
...
export function exinfoFor(name: string, sectionTag?: string): ExInfo {
  if (EXINFO[name]) return EXINFO[name];
  const byTag: Record<string, ExInfo> = {
    joints: { m: ['mobility', 'balance'], alt: [] },
    hipext: { m: ['glutes', 'hamstrings'], alt: [] },
    fascia: { m: ['mobility'], alt: [] },
  };
  return (sectionTag ? byTag[sectionTag] : undefined) || { m: ['mobility'], alt: [] };
}
