...
interface DemoEntry { d: string; t: string }
const DEMOS: Record<string, DemoEntry> = {
  ...
  "Glute bridge with posterior tilt": {"d":"Butt_Lift_Bridge","t":"Butt Lift (Bridge)"},
  "Kneeling hip flexor stretch with posterior tilt": {"d":"Kneeling_Hip_Flexor","t":"Kneeling Hip Flexor"},
  "Dead bug with heel slide": {"d":"Dead_Bug","t":"Dead Bug"},
  "Reverse crunch": {"d":"Reverse_Crunch","t":"Reverse Crunch"},
  "RKC plank": {"d":"Plank","t":"Plank"},
  "Hanging or captain's-chair knee raise": {"d":"Knee_Hip_Raise_On_Parallel_Bars","t":"Knee/Hip Raise On Parallel Bars"},
  "Standing banded knee drive": {"d":"Hip_Flexion_with_Band","t":"Hip Flexion with Band"},
  "Standing band march, tall": {"d":"Hip_Flexion_with_Band","t":"Hip Flexion with Band"},
  "Rectus femoris stretch": {"d":"Quad_Stretch","t":"Quad Stretch"},
  "Thoracic extension over roller, upper back only": {"d":"Upper_Back_Stretch","t":"Upper Back Stretch"},
  "Suitcase carry, pelvis neutral": {"d":"Farmers_Walk","t":"Farmer's Walk"},
  "Split squat with deliberate posterior tilt": {"d":"Split_Squat_with_Dumbbells","t":"Split Squat with Dumbbells"},
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
interface VideoEntry { v: string; t: string }
...
export type Demo =
  | { kind: 'frames'; frames: [string, string]; title: string }
  | { kind: 'video'; src: string; watch: string; title: string };

export function demoFor(name: string): Demo | null {
  const e = DEMOS[name];
  if (e) {
    return {
      kind: 'frames',
      frames: [DEMO_BASE + e.d + '/0.jpg', DEMO_BASE + e.d + '/1.jpg'] as [string, string],
      title: e.t,
    };
  }
  const v = VIDEOS[name];
  if (v) {
    ...
    return {
      kind: 'video',
      src: `https://www.youtube-nocookie.com/embed/${v.v}?${q}`,
      watch: `https://www.youtube.com/watch?v=${v.v}`,
      title: v.t,
    };
  }
  return null;
